...
import logging
import requests
from config import Config, ORCHESTRATOR_URL, AGENT_VERSION
...
def main():
    ...
    logger.info("Orchestrator base URL: %s", ORCHESTRATOR_URL)
    try:
        ping_url = f"{ORCHESTRATOR_URL.rstrip('/')}/api/agents/ping"
        res = requests.get(ping_url, timeout=5)
        logger.info("Ping orchestrator (%s): %s %s", ping_url, res.status_code, res.text)
    except Exception as exc:
        logger.warning("Ping orchestrator failed: %s", exc)
