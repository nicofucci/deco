...
class APIClient:
    ...
    def send_heartbeat(self, payload):
        ...
        try:
            # Aumentamos timeout a 30s para conexiones lentas
            res = self.session.post(url, json=payload, headers=self._get_headers(), timeout=30)
            if res.status_code == 200:
                return res.json() # Returns dict with 'pending_jobs'
            elif res.status_code == 404:
                logger.warning("Heartbeat returned 404 (Agent not found). Resetting Agent ID to force re-registration.")
                self.config.set_agent_id(None)
                return None
            else:
                logger.warning(f"Heartbeat failed. Status: {res.status_code}. Body: {res.text[:200]}")
                return None
        except Exception as e:
