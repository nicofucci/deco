    ...
    def _run_logic(self, job):
        ...
        try:
            ...
            if job_type == "scan_local_ports":
                ...
                logger.info("Running X-RAY Network Scan...")
                from .xray_service import XRayScanner
                scanner = XRayScanner()
                
                # 1. Identify Network
                network_info = scanner.get_local_network_info()
                if not network_info:
                    raise Exception("Could not identify local network")
                
                cidr = network_info["cidr"]
                logger.info(f"Target Subnet: {cidr}")

                # 2. Discovery
                hosts = scanner.scan_subnet(cidr)
                
                # 3. Enrichment
                enriched_hosts = scanner.resolve_details(hosts)
                
                result["data"] = {
                    "network": {
                        "cidr": cidr,
                        "gateway": str(network_info['network'].network_address + 1), # guesstimate
                        "interface": network_info["ip"]
                    },
                    "devices": enriched_hosts,
