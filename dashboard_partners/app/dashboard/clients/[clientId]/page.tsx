...
export default function ClientControlPage() {
    ...
    const handleViewResult = async (job: any) => {
        setActionLoading(true);
        const token = localStorage.getItem("deco_partner_api_key");
        if (!token) return;
        try {
            const res = await getScanJobResult(token, clientId, job.id);
            setJsonModalTitle(`Resultado: ${job.type} (${job.id.substring(0, 8)})`);
            setJsonModalData(res);
            setJsonModalOpen(true);
        } catch (e) {
            console.error(e);
            alert("Error al cargar resultado JSON");
        } finally {
            setActionLoading(false);
        }
    };

    const handleJobReport = async (job: any) => {
        if (!confirm(`¿Generar reporte técnico basado en este escaneo (${job.id.substring(0, 8)})?`)) return;
        ...
        try {
            // Trigger generation linked to scan_id
            const report = await generateClientReport(token, clientId, "technical", job.id);

            // Poll for readiness
            alert("Generando reporte histórico... Esto puede tardar unos segundos.");

            // Simple wait loop (MVP)
            let readyReport = null;
            for (let i = 0; i < 10; i++) {
                await new Promise(r => setTimeout(r, 2000));
                // Fetch reports list to find this one updated? Or check generic list?
                // Ideally we'd have a 'getReportStatus' but let's reuse getClientReports or assume list updates
                const reps = await getClientReports(token, clientId);
                const match = reps.find((r: any) => r.id === report.id);
                if (match && match.status === 'ready' && match.download_url) {
                    readyReport = match;
                    break;
                }
            }
            ...
            if (readyReport) {
                await handleDownloadReport(readyReport.download_url);
            } else {
                alert("El reporte se está generando en segundo plano. Revise la pestaña 'Reportes' en unos momentos.");
            }
            ...
            console.error(e);
            alert("Error al solicitar reporte del trabajo");
        } finally {
            setActionLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-slate-950 pb-20 p-6">
            ...
            <tbody className="divide-y divide-slate-800">
                        ...
                        <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-2">
                                <button
                                    ...
                                    className="p-1 px-2 bg-slate-800 hover:bg-slate-700 text-blue-400 text-xs rounded border border-slate-700 transition flex items-center gap-1"
                                    title="Ver JSON Raw"
                                >
                                ...
                                {job.status === 'done' && (
                                    <button
                                        onClick={() => handleJobReport(job)}
                                        className="p-1 px-2 bg-slate-800 hover:bg-slate-700 text-green-400 text-xs rounded border border-slate-700 transition flex items-center gap-1"
                                        title="Descargar Reporte Técnico Relacionado"
                                    >
                ...
                import {
                    ...
                    Smartphone, Printer, HelpCircle, Bug, Flame, TrendingUp, Zap, CheckSquare, XSquare, PlayCircle, Eye, Calendar
                } from "lucide-react";
...
const getDeviceIcon = (type: string) => {
    ...
    const loadData = async () => {
                ...
                const summary = await getClientSummary(token, clientId);
                                setClientData(summary.client || summary); // Handle nested or flat
            } catch (err) {
                                                        ...
                                                        {!reportsLoading && (
                                                            <div className="space-y-2">
                                                                {reports.length === 0 ? (
                                                                    ...
                                                                    reports.map((rep: any) => (
                                                                        <div key={rep.id} className="flex justify-between items-center p-4 bg-slate-900 border border-slate-800 rounded-lg hover:border-slate-700 transition-colors">
                                                                            <div className="flex items-start gap-4">
                                                                                <div className="p-3 bg-slate-800 rounded text-slate-400">
                                                                                    <FileText className="h-6 w-6" />
                                                                                </div>
                                                                                <div>
                                                                                    <div className="flex items-center gap-2">
                                                                                        <span className="text-white font-semibold capitalize">{rep.type || "Reporte"}</span>
                                                                                        <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${rep.status === 'ready' ? 'bg-green-900/30 text-green-400' : 'bg-yellow-900/30 text-yellow-400'
                                                                                            }`}>{rep.status || 'unknown'}</span>
                                                                                    </div>
                                                                                    <div className="text-xs text-slate-500 mt-1 flex gap-4">
                                                                                        <span><Calendar className="w-3 h-3 inline mr-1" /> {new Date(rep.generated_at).toLocaleString()}</span>
                                                                                        {rep.risk_score !== undefined && (
                                                                                            <span className={`${rep.risk_score >= 80 ? 'text-green-400' : rep.risk_score >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                                                                                                <Shield className="w-3 h-3 inline mr-1" /> Score: {rep.risk_score}/100
                                                                                            </span>
                                                                                        )}
                                                                                    </div>
                                                                                    <div className="text-xs text-slate-400 mt-1 font-mono">
                                                                                        {rep.summary || `Assets: ${rep.assets_count} | Findings: ${rep.findings_count}`}
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                            <button
                                                                                onClick={() => handleDownloadReport(rep.download_url)}
                                                                                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-medium flex items-center gap-2 shadow-lg shadow-blue-900/20"
                                                                            >
