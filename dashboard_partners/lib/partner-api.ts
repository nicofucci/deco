// Single source of truth: always go through the Next.js proxy (same origin)
// so SSR and browser share the exact same base.
...
export async function getMyAgents(token: string) {
    const res = await fetch(`${API_URL}/api/partners/me/agents`, {
        headers: { "X-Partner-API-Key": token },
    });
...
export async function updateMyAgents(token: string, version: string = "latest") {
    const res = await fetch(`${API_URL}/api/partners/me/agents/update`, {
        method: "POST",
        headers: getHeaders(token),
        body: JSON.stringify({ version }),
    });
    if (!res.ok) throw new Error("Error al actualizar agentes");
    return res.json();
}
...
export async function generateClientReport(token: string, clientId: string, type: string, scanId?: string) {
    const res = await fetch(`${API_URL}/api/partners/me/clients/${clientId}/reports`, {
        method: "POST",
        headers: getHeaders(token),
        body: JSON.stringify({ type, scan_id: scanId }),
    });
    if (!res.ok) throw new Error("Error al generar reporte");
    return res.json();
}

export async function getScanJobResult(token: string, clientId: string, jobId: string) {
    const res = await fetch(`${API_URL}/api/partners/me/clients/${clientId}/jobs/${jobId}/result`, {
        headers: { "X-Partner-API-Key": token },
    });
    if (!res.ok) throw new Error("Error al obtener resultados del job");
    return res.json();
}
