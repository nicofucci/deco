...
def register_agent(
    ...
    if existing_agent:
        ...
        if getattr(payload, "capabilities", None):
            existing_agent.capabilities = payload.capabilities
    ...
    # --- BOOTSTRAP SCAN (INCIDENTE_CLIENTE_SIN_DATOS) ---
    # Si el cliente es nuevo (no tiene jobs), lanzamos un scan inicial automático
    # para que la consola no esté vacía.
    try:
        existing_jobs = db.query(ScanJob).filter(ScanJob.client_id == client.id).count()
        if existing_jobs == 0:
            logger.info(f"[BOOTSTRAP] Cliente {client.id} nuevo. Creando Scan Inicial...")
            # Detect primary CIDR or fallback
            target = payload.ip if hasattr(payload, 'ip') and payload.ip else "192.168.1.0/24"
            # Try to grab CIDR from payload if available
            # Note: payload might not have detailed network info yet, usually comes in first heartbeat
            # But we want to queue it now.
            
            bootstrap_job = ScanJob(
                client_id=client.id,
                agent_id=agent.id,
                type="xray_network_scan", # Using X-RAY for rich initial data
                target=target, 
                status="pending",
                created_at=datetime.now(timezone.utc),
                params={"mode": "fast_discovery"}
            )
            db.add(bootstrap_job)
            db.commit()
            logger.info(f"[BOOTSTRAP] Job {bootstrap_job.id} creado para agente {agent.id}")
    except Exception as e:
        logger.error(f"[BOOTSTRAP] Error creando scan inicial: {e}")
    # ----------------------------------------------------
    ...
    return [
        {
            "id": a.id,
            "hostname": a.hostname,
            "status": compute_agent_online_status(a),  # Use computed status
