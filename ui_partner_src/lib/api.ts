...
export async function getClientNetworkAssets(token: string, clientId: string, scope: string = "lan") {
    const res = await fetch(`${API_URL}/api/network/clients/${clientId}/network-assets?scope=${scope}`, {
        headers: { "X-Partner-API-Key": token }
    });
...
export async function getAssetEvidence(token: string, clientId: string, assetId: string) {
    const res = await fetch(`${API_URL}/api/network/clients/${clientId}/network-assets/${assetId}/evidence`, {
        headers: { "X-Partner-API-Key": token }
    });
    if (!res.ok) throw new Error("Failed to fetch asset evidence");
    return res.json();
}
